DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `usp_GenerateAprioriRules`(IN GARminSupport DECIMAL(5,2), IN GARconfidence DECIMAL(5,2))
BEGIN
...
group by itemName) C
WHERE C.itemsupport/transactionCount >= GARminSupport;


-- STEP SIX: Call Stored Procedure to get Level2 element candidates.
CALL storedemo.usp_getL2Candidates(GARminSupport, transactionCount, 1);

-- STEP SEVEN: Call Stored Procedure to get Level3 and above element candidates.
CALL storedemo.usp_getCandidates(GARminSupport, transactionCount, 2);
...
WHERE Confidence>= GARconfidence;
